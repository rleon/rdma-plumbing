...
def sources():
    ...
    for I in os.listdir(git_root):
        if I.startswith("lib") or I == "iwpm" or I == "srptools":
...
def do_cpp(src,ofn,cmd):
    ...
    args.update(["-O2","-DNVALGRIND","--std=gnu99"]);
...
def cmp_elf(fromfn,tofn):
    ...
    soname = "NONE";
    deps = set();
    for I in subprocess.check_output(["readelf","-d",fromfn]).splitlines():
    ...
    with open(os.path.join(dest,"FILES"),"wt") as F:
        ...
        for fn in fns:
            ...
            if (fn.endswith(".h") or
                ...
                fn.endswith(".sh") or
                fn.endswith(".cmds") or
                fn.endswith("rxe_cfg") or
                fn.endswith("srpd") or
                fn.endswith("srp_daemon")):
                cmp_content(fn,tofn);
            elif ".so" in fn or "/bin/" in fn or "/sbin/" in fn:
                cmp_elf(fn,tofn);
            else:
